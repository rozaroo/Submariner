...
public class WorldMapManager : MonoBehaviour
{
    ...
    [SerializeField] private float minDistanceBetweenEvents = 10f; //Minimal Limit
    ...
    [SerializeField] private WorldMapUIElementEventChannelSO onWorldElementGenerated; //Not yet Used, only for individual items.
    ...
    } //NOTE: This method is used only at the start of the game, if you want to generate individual elements use the GenerateIndividualElement method.
    ...
    private GameObject GenerateIndividualElement(WorldMapElementSO mapElementSo)
    {
        ...
        if (elementGo != null)
        {
            ...
            Destroy(elementGo);
            Log.Warning("[WorldMapManager] Could not find a valid position for the new event.");
            return null;
        }
        Log.Warning("[WorldMapManager] No world events available to generate.");
        return null;
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            Gizmos.DrawWireCube(worldContainerGo.transform.position, new Vector3(worldSize, 0, worldSize));   
        }
    }
